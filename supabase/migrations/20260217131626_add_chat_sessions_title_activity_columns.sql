...
DO $$
BEGIN
  IF to_regclass('public.chat_sessions') IS NULL OR to_regclass('public.messages') IS NULL THEN
    RAISE NOTICE 'Skipping migration 20260217131626_add_chat_sessions_title_activity_columns: required tables do not exist yet.';
    RETURN;
  END IF;
