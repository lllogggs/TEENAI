...
const Auth: React.FC<AuthProps> = ({ onLogin, loading }) => {
  ...
  const termsContent = (
    <div className="space-y-3">
      <p><strong>제1조 (목적)</strong><br />본 약관은 포틴 AI(이하 "회사")가 제공하는 인공지능 채팅 서비스(이하 "서비스")의 이용 조건 및 절차를 규정합니다.</p>
  ...
  if (view === 'selection') {
    return (
      <div className="min-h-[100dvh] bg-[#F1F5F9] flex flex-col items-center justify-center p-4 md:p-10 pt-[max(env(safe-area-inset-top,0px),1rem)] pb-[max(env(safe-area-inset-bottom,0px),1rem)]">
        <div className="text-center mb-8 md:mb-20 animate-in fade-in zoom-in duration-700 select-none">
          <h1 className="text-5xl md:text-7xl font-black text-brand-900 tracking-tighter mb-3 md:mb-6">포틴 AI</h1>
          <p className="text-slate-500 font-bold text-sm md:text-lg">청소년을 위한 가장 안전한 AI 성장의 공간</p>
        </div>

        <div className="mx-auto w-full max-w-sm md:max-w-4xl lg:max-w-5xl px-4 flex flex-col md:flex-row gap-5 md:gap-6 lg:gap-10">
          ...
          <button onClick={() => setView('parent-auth')} className="flex-1 group flex flex-col items-center justify-between p-8 md:p-10 lg:p-14 bg-white rounded-[2rem] md:rounded-[3rem] shadow-2xl hover:shadow-slate-200 hover:-translate-y-2 transition-all text-center border border-white relative overflow-hidden">
            ...
            <div className="w-16 h-16 md:w-20 md:h-20 bg-indigo-50 rounded-2xl md:rounded-3xl flex items-center justify-center text-3xl md:text-5xl mb-5 md:mb-8 group-hover:scale-110 transition-transform">🛡️</div>
  ...
  return (
    <div className="min-h-[100dvh] bg-[#F1F5F9] flex items-center justify-center p-4 xl:p-10 pt-[max(env(safe-area-inset-top,0px),1rem)] pb-[max(env(safe-area-inset-bottom,0px),1rem)]">
