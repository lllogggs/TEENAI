...
const StudentChat: React.FC<StudentChatProps> = ({ user, onLogout }) => {
  ...
  const handleSend = async () => {
    ...
    const isDanger = DANGER_KEYWORDS.some(k => userText.includes(k));
    if (isDanger) {
    ...
    try {
      ...
      const aiMsg: ChatMessage = { 
        role: 'model', 
        text: data.text || "잠시 대화가 어려워요. 다시 시도해볼까요?", 
        timestamp: Date.now() 
      };
      ...
      <div className="flex-1 overflow-y-auto p-10 space-y-10 bg-slate-50/50 custom-scrollbar pb-40">
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-500`}>
            <div className={`max-w-[75%] p-7 rounded-[2.25rem] text-[15px] leading-relaxed shadow-sm font-medium tracking-tight ${
              m.role === 'user' 
