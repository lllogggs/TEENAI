...
const StudentChat: React.FC<StudentChatProps> = ({ user, onLogout }) => {
  ...
  const [pendingMode, setPendingMode] = useState<'대화' | '공부' | null>(null);
  ...
  const handleModeChange = (nextMode: '대화' | '공부') => {
    ...
    setIsModeSwitchModalOpen(true);
  };

  const closeModeSwitchModal = () => {
    setIsModeSwitchModalOpen(false);
    setPendingMode(null);
  };
  ...
  return (
    <div className="flex h-[100dvh] bg-[radial-gradient(circle_at_top,#ffffff_0%,#f8fafc_48%,#eef4ff_100%)] flex-col overflow-hidden">
                ...
                <button
                  ...
                  className={`group flex h-11 shrink-0 items-center justify-center gap-2 overflow-hidden rounded-2xl border shadow-sm transition-all hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-200 ${
                    isSidebarOpen
                      ...
                      : 'w-11 border-slate-200 bg-white text-slate-700 hover:border-brand-200 hover:bg-brand-50 hover:text-brand-800'
                  }`}
                >
                  {isSidebarOpen ? (
              ...
              <div className="pb-24 md:pb-28 max-w-3xl mx-auto w-full">
                {messages.map((m, i) => (
                  <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-500`}>
                    <div
                      className={`max-w-[78%] md:max-w-[70%] xl:max-w-[64%] px-4 py-3.5 md:px-5 md:py-[1.125rem] rounded-[1.6rem] text-[16px] md:text-[17px] leading-[1.75] shadow-sm font-semibold tracking-[-0.01em] whitespace-pre-wrap ${m.role === 'user'
                        ? 'bg-gradient-to-br from-brand-900 to-[#312e81] text-white rounded-tr-[0.45rem] shadow-brand-900/15'
                        : 'bg-white/95 text-slate-800 border border-white rounded-tl-[0.45rem] shadow-lg shadow-slate-200/60 ring-1 ring-slate-100/70'
                        }`}
                    >
      ...
      <ModeSwitchConfirmModal
        ...
        currentModeLabel={chatMode}
        nextModeLabel={pendingMode || chatMode}
        onClose={closeModeSwitchModal}
