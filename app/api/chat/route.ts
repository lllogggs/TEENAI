...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.warn('generateContent failed, falling back to chat.sendMessage', { primaryError });

      const chatHistory = normalizeHistory(history).map((line) => {
        const isUser = line.startsWith('학생: ');
        return {
          role: isUser ? 'user' : 'model',
          parts: [{ text: line.replace(/^(학생|멘토):\s*/, '') }],
        };
      });
      ...
      const chatResult = await chat.sendMessage({ message: userText });
      rawText = chatResult.text || '';
    }

    const finalText = applyLengthPostProcess(rawText, targetLength);
